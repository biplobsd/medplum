...
function parseIndexDefinition(indexdef: string): IndexDefinition {
  ...
  return {
    ...
    gin: indexdef.includes('USING gin'),
...
function buildCreateTables(result: SchemaDefinition, resourceType: string, fhirType: TypeSchema): void {
  ...
  const tableDefinition: TableDefinition = {
    ...
    indexes: [
      { columns: ['lastUpdated'] },
      { columns: ['compartments'], gin: true },
      { columns: ['_source'] },
      { columns: ['_tag'], gin: true },
      { columns: ['_profile'], gin: true },
    ],
  };
  ...
  result.tables.push({
    ...
    indexes: [{ columns: ['id'] }, { columns: ['lastUpdated'] }],
  });
  ...
  result.tables.push({
    ...
    indexes: [{ columns: ['resourceId'] }, { columns: ['code'] }, { columns: ['system'] }, { columns: ['value'] }],
  });
}
...
function buildSearchColumns(tableDefinition: TableDefinition, resourceType: string): void {
  for (const searchParam of searchParams) {
    ...
    tableDefinition.indexes.push({ columns: [columnName], gin: details.array });
  }
}
...
function buildSearchIndexes(result: TableDefinition, resourceType: string): void {
  if (resourceType === 'User') {
    result.indexes.push({ columns: ['email'], unique: true });
  }
}
...
function buildLookupTable(result: SchemaDefinition, tableName: string, columns: string[]): void {
  ...
  for (const column of columns) {
    ...
    tableDefinition.indexes.push({ columns: [column] });
  }
...
function buildValueSetElementTable(result: SchemaDefinition): void {
  result.tables.push({
    ...
    indexes: [{ columns: ['system'] }, { columns: ['code'] }, { columns: ['display'] }],
  });
}
...
function migrateColumns(b: FileBuilder, startTable: TableDefinition, targetTable: TableDefinition): void {
  for (const targetColumn of targetTable.columns) {
    ...
    if (!startColumn) {
      ...
      startColumn.type.replaceAll('TIMESTAMP WITH TIME ZONE', 'TIMESTAMPTZ').trim() !==
      targetColumn.type.replaceAll(' PRIMARY KEY', '').replaceAll(' DEFAULT FALSE', '').trim()
    ) {
